...
import com.vinayak.workmanagerguide.ui.theme.WorkManagerGuideTheme
            ...
            WorkManagerGuideTheme {
                ...
                Column(
                    ...
                    when(downloadInfo?.state) {
                        ...
                        WorkInfo.State.SUCCEEDED -> Text("Download completed")
                        ...
                        WorkInfo.State.BLOCKED -> Text("Download not started")
                    }
                    Spacer(modifier = Modifier.height(8.dp))
                    when(filterInfo?.state) {
                        WorkInfo.State.RUNNING -> Text("Applying filter...")
                        WorkInfo.State.SUCCEEDED -> Text("Filter completed")
                        ...
                        WorkInfo.State.BLOCKED -> Text("Filter not started")
                    }
                }
            }
        }
